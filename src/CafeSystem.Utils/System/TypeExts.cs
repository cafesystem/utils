...
public static class TypeExts
{
    ...
    public static ConstructorInfo FindConstructor(this Type type, params object[] args)
    {
        ...
        bool MatchSignature(ConstructorInfo constructorInfo)
        {
            ...
            parameters.ForEach((p, i) =>
                match &= i < args.Length ? p.ParameterType.IsInstanceOfType(args[i]) : p.HasDefaultValue);
            return match;
        }
    }
    ...
    {
        return typeof(TypeExts).GetMethods()
            .Single(x => x.Name == "Default" && x.IsGenericMethod)
